...
socket.on("load", function (data) {
    ...
    for (var i = 0; i < temp.length; i++) {
        ...
        circle.radius = temp[i].radius;
        circle.element = temp[i].element;
        circle.color = temp[i].color;
        circle.team = temp[i].team;
        circle.hit = temp[i].hit;
        circle.timer = temp[i].timer;
        circle.attacking = temp[i].immune;
        circle.immune = temp[i].radius;
        circle.totalTime = temp[i].totalTime;
        circle.elapsedTime = temp[i].elapsedTime;
        circle.x = temp[i].x;
        circle.y = temp[i].y;
        circle.velocity = temp[i].velocity;
        circle.speed = temp[i].speed;
        circle.startX = temp[i].startX;
        circle.startY = temp[i].startY;
        circle.ratio = temp[i].ratio;
...
function save(event) {
    ...
    var saveData = [];
    for (var i = 0; i < gameEngine.entities.length; i++) {
        tempRadius = this.gameEngine.entities[i].radius;
        tempElement = this.gameEngine.entities[i].element;
        tempColor = this.gameEngine.entities[i].color;
        tempTeam = this.gameEngine.entities[i].team;
        tempHit = this.gameEngine.entities[i].hit;
        tempTimer = this.gameEngine.entities[i].timer;
        tempAttacking = this.gameEngine.entities[i].attacking;
        tempImmune = this.gameEngine.entities[i].immune;

        tempStartX = this.gameEngine.entities[i].startX;
        tempStartY = this.gameEngine.entities[i].startY;
        tempTotalTime = this.gameEngine.entities[i].totalTime;
        tempElapsedTime = this.gameEngine.entities[i].elapsedTime;
        tempX = this.gameEngine.entities[i].x;
        tempY = this.gameEngine.entities[i].y;
        tempVelocity = this.gameEngine.entities[i].velocity;
        tempSpeed = this.gameEngine.entities[i].speed;
        tempRatio = this.gameEngine.entities[i].ratio;

        saveData.push({radius: tempRadius, element: tempElement, color: tempColor, team: tempTeam, hit: tempHit, timer: tempTimer
            , attacking: tempAttacking, immune: tempImmune, totalTime: tempTotalTime, elapsedTime: tempElapsedTime, x: tempX, y: tempY
            , velocity: tempVelocity, speed: tempSpeed});
    }
    console.log(saveData.length);
    socket.emit("save", { studentname: "Casey Anderson", statename: "aState", data: saveData});
    
}
function load(event) {
    console.log("load Game");
    socket.emit("load", { studentname: "Casey Anderson", statename: "aState"});
